...
from transformers import SwinModel, LlamaForCausalLM, LlamaTokenizer
...
class R2GenEVA(pl.LightningModule):
    """
    R2GenEVA model.
    """
    def __init__(self, args):
        ...
        if args.projector=='mlp':
            self.llama_proj = MLP(self.visual_encoder.num_features, 2048, self.llama_model.config.hidden_size)
        elif args.projector=='fc':
            self.llama_proj = nn.Linear(self.visual_encoder.num_features, self.llama_model.config.hidden_size)

        self.mapper_vis = nn.Linear(self.llama_model.config.hidden_size, 512) 
        self.mapper_text = nn.Linear(self.llama_model.config.hidden_size, 512)
    ...
    def encode_img(self, images):
        ...
        for image in images:
            ...
            if self.hparams.global_only:
                ...
                image_embed_mat = visual_outputs['pooler_output'].unsqueeze(1).to(device)
            image_embeds.append(image_embed)
            image_embeds_mat.append(image_embed_mat.squeeze())
            
        image_embeds = torch.stack(image_embeds).mean(0)
        image_matching = torch.stack(image_embeds_mat).mean(0)

        inputs_llama = self.llama_proj(image_embeds)
        image_matching = self.llama_proj(image_matching) # m2t

        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(image.device)
        return inputs_llama, atts_llama, image_matching
    ...
    def VTA_loss(self, image_embeds, text_embeds): # image_embeds: [bs, 4096], text_embeds: [bs, 4096]
        # normalized features
    ...
    def forward(self, samples):
        image = samples["image"]
        img_embeds, atts_img, image_matching = self.encode_img(image)
        ...
        GT_report_feat_align = torch.mean(to_regress_embeds.to(torch.float32), dim=1)
        image_matching_former = self.mapper_vis(image_matching)
        GT_report_feat_align = self.mapper_text(GT_report_feat_align)
        vta_loss = self.VTA_loss(image_matching_former, GT_report_feat_align)

        all_loss = lm_loss + vta_loss * 0.2
